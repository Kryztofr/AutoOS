...
$activeAdapter = Get-NetAdapter | Where-Object { $activeAdapter.PhysicalMediaType -eq "802.3" -and $activeAdapter.Status -eq "Up" } | Select-Object -First 1
...
if ($activeAdapter) {
    ...
    $wakeOnMagicPacket = $adapterProperties | Where-Object { $activeAdapter.DisplayName -eq "Wake on Magic Packet" }
    if ($wakeOnMagicPacket -and $wakeOnMagicPacket.DisplayValue -ne "Enabled") {
        $status = $false
    }

    # check wake from s0ix on magic packet
    $wakeFromS0ix = $adapterProperties | Where-Object { $activeAdapter.DisplayName -eq "Wake from S0ix on Magic Packet" }
    if ($wakeFromS0ix -and $wakeFromS0ix.DisplayValue -ne "Enabled") {
        $status = $false
    }

    # check wake on magic packet from s5
    $wakeFromS5 = $adapterProperties | Where-Object { $activeAdapter.DisplayName -eq "Wake On Magic Packet From S5" }
    if ($wakeFromS5 -and $wakeFromS5.DisplayValue -ne "Enabled") {
        $status = $false
    }

    # check wake on magic packet when system is in the S0ix power state
    $wakeFromS0ix2 = $adapterProperties | Where-Object { $activeAdapter.DisplayName -eq "Wake on magic packet when system is in the S0ix power state" }
    if ($wakeFromS0ix2 -and $wakeFromS0ix2.DisplayValue -ne "Enabled") {
        $status = $false
    }

    # check enable pme
    $enablePME = $adapterProperties | Where-Object { $activeAdapter.DisplayName -eq "Enable PME" }
...
Get-WmiObject -Class Win32_NetworkAdapter | Where-Object { $activeAdapter.PNPDeviceID -ne $null } | ForEach-Object {
    ...
    if ($physicalMediaType -eq "14") {
        $matchingWakeDevice = $deviceWakeEnable | Where-Object { $activeAdapter.InstanceName -like "*$pnpDeviceId*" }
...
Get-WmiObject -Class Win32_NetworkAdapter | Where-Object { $activeAdapter.PNPDeviceID -ne $null } | ForEach-Object {
    ...
    $physicalMediaType = (Get-ItemProperty -Path $classKey -Name "*PhysicalMediaType" -ErrorAction SilentlyContinue)."*PhysicalMediaType" 
    if ($physicalMediaType -eq "14") {
        $matchingWakeDevice = $deviceWakeOnMagicPacketOnly | Where-Object { $activeAdapter.InstanceName -like "*$pnpDeviceId*" }
        if ($matchingWakeDevice) {
            foreach ($device in $matchingWakeDevice) {
