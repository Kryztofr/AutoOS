...
public static class ApplicationStage
{
	...
	public static List<(string Title, Func<Task> Action, Func<bool> Condition)> GetActions(IStatusReporter reporter = null, ApplicationSelection selection = null)
	{
		...
		var actions = new List<(string Title, Func<Task> Action, Func<bool> Condition)>
		{
			...
			("Downloading LosslessCut", async () => await DownloadHelper.Download(JsonDocument.Parse(await new HttpClient { DefaultRequestHeaders = { { "User-Agent", "AutoOS" } } }.GetStringAsync("https://api.github.com/repos/chrishuan9/LosslessCut-winget-rls/releases")).RootElement.EnumerateArray().First(release => !release.GetProperty("prerelease").GetBoolean() && release.GetProperty("assets").EnumerateArray().Any(asset => asset.GetProperty("name").GetString().Contains(".zip"))).GetProperty("assets").EnumerateArray().First(asset => asset.GetProperty("name").GetString().Contains(".zip")).GetProperty("browser_download_url").GetString(), Path.GetTempPath(), "losslesscut.zip", reporter: reporter), () => LosslessCut == true),

			// install losslesscut
			("Installing LosslessCut", async () => await ExtractHelper.Extract(Path.Combine(Path.GetTempPath(), "losslesscut.zip"), Path.Combine(Path.GetTempPath(), "LosslessCut")), () => LosslessCut == true),
			...
			("Cleaning up LosslessCut files", async () => File.Delete(Path.Combine(Path.GetTempPath(), "losslesscut.zip")), () => LosslessCut == true),
