...
public static class SecurityStage
{
	public static async Task<List<(string Title, Func<Task> Action, Func<bool> Condition)>> GetActions()
	{
		...
		return new List<(string Title, Func<Task> Action, Func<bool> Condition)>
		{
			...
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => ServicesHelper.KillServiceProcess("wscsvc")), () => WindowsDefender == false),
			...
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => ServicesHelper.KillServiceProcess("webthreatdefsvc")), () => WindowsDefender == false),
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => ServicesHelper.KillServiceProcess("webthreatdefusersvc")), () => WindowsDefender == false),
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => ServicesHelper.KillServiceProcess("wscsvc")), () => WindowsDefender == false),
			("Disabling Windows Defender", async () => RegistryHelper.RunAs(RegistryHelper.Identity.TrustedInstaller, () => File.Move(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "SecurityHealthService.exe"), Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "SecurityHealthService.exee")), () => WindowsDefender == false),
