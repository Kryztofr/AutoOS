...
public static class GraphicsStage
{
    ...
    public static async Task<List<(string Title, Func<Task> Action, Func<bool> Condition)>> GetActions()
    {
        ...
        var actions = new List<(string Title, Func<Task> Action, Func<bool> Condition)>
        {
			...
			("Installing MSI Afterburner", async () => await ProcessActions.RunPowerShell(@"$Shell=New-Object -ComObject WScript.Shell; $msiPath=[System.IO.Path]::Combine(${env:ProgramFiles(x86)}, 'MSI Afterburner'); @(@{P='MSI Afterburner.lnk';T=[System.IO.Path]::Combine($msiPath, 'MSIAfterburner.exe')},@{P='ReadMe.lnk';T=[System.IO.Path]::Combine($msiPath, 'Doc', 'ReadMe.pdf')},@{P='Uninstall.lnk';T=[System.IO.Path]::Combine($msiPath, 'Uninstall.exe')},@{P='SDK\MSI Afterburner localization reference.lnk';T=[System.IO.Path]::Combine($msiPath, 'SDK', 'Doc', 'Localization reference.pdf')},@{P='SDK\MSI Afterburner skin format reference.lnk';T=[System.IO.Path]::Combine($msiPath, 'SDK', 'Doc', 'USF skin format reference.pdf')},@{P='SDK\Samples.lnk';T=[System.IO.Path]::Combine($msiPath, 'SDK', 'Samples\')}) | % {$Shortcut=$Shell.CreateShortcut([System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\MSI Afterburner', $_.P)); $Shortcut.TargetPath=$_.T; $Shortcut.Save()}"), null),
            ...
            ("Installing OBS Studio", async () => await ProcessActions.RunPowerShell(@"$s=New-Object -ComObject WScript.Shell;$obsPath=[System.IO.Path]::Combine($env:ProgramFiles, 'obs-studio', 'bin', '64bit');$sc=$s.CreateShortcut([System.IO.Path]::Combine($env:ProgramData,'Microsoft\Windows\Start Menu\Programs\OBS Studio.lnk'));$sc.TargetPath=[System.IO.Path]::Combine($obsPath, 'obs64.exe');$sc.WorkingDirectory=$obsPath;$sc.Save()"), null)
        };
