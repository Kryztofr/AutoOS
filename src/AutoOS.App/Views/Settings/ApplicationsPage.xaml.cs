...
public sealed partial class ApplicationsPage : Page
{
	...
	private void GetItems()
	{
		...
		var launchersList = new List<GridViewItem>
		{
			...
			new() { Text = "NoRisk Client", ImageSource = "ms-appx:///Assets/Fluent/NoRiskClient.png", IsInstalled = Directory.GetDirectories(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "NoRisk Launcher").SelectMany(directory => Directory.GetFiles(directory, "noriskclient-launcher-v*.exe")).Any() },
		...
		var miscellaneousList = new List<GridViewItem>
		{
			...
			new() { Text = "CrystalDiskInfo", ImageSource = "ms-appx:///Assets/Fluent/CrystalDiskInfo.png", IsInstalled = File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "CrystalDiskInfo", "DiskInfo64.exe")) },
